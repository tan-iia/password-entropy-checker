...
const commonWarningText = document.getElementById("commonWarning");

let commonPasswords = new Set();
async function loadCommonPasswords() {
  const url = "https://raw.githubusercontent.com/edeniyanda/Python-Password-Strength-Checker/main/common.txt";
  try {
    ...
    console.error("Failed to load common list:", err);
  }
}
loadCommonPasswords();
...
input.addEventListener("input", () => {
  ...
  let strengthLabel = getStrength(bits);
  if (isCommon) {
    strengthLabel += " — Common Password!";
  }
  ...
  commonWarningText.textContent = isCommon
    ? "⚠ This password appears in a common password list. Consider choosing a stronger one!"
    : "";
